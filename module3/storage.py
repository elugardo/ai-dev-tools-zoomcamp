...
transaction seam from :mod:`database`: SQLite serializes writers, PostgreSQL
relies on the row locks taken here (``FOR UPDATE``, ``SKIP LOCKED`` for
claims), always task first and attempt second.
"""
...
def create_task(sender_id: str, recipient_id: str, input_text: str, idempotency_key: str | None) -> dict[str, str]:
    # On SQLite the serialized writer makes the sender-scoped idempotency check
    # and the unique constraint one operation even when two API processes
    # race.  On PostgreSQL the loser of that race hits the unique constraint;
    # it then re-runs the lookup and answers like any retry.
    try:
        with immediate_transaction() as db:
            return _create_task(db, sender_id, recipient_id, input_text, idempotency_key)
...
def _create_task(
    db: Session, sender_id: str, recipient_id: str, input_text: str, idempotency_key: str | None
) -> dict[str, str]:
...
def claim_one(agent_id: str, worker_id: str | None) -> dict[str, Any] | None:
    with immediate_transaction() as db:
        ...
        # SKIP LOCKED lets concurrent PostgreSQL claims take different rows
        # instead of queueing on the same one; SQLite ignores the clause.
        task = db.scalar(
            select(Task)
...
def _find_attempt_for_token(db: Session, task_id: str, token: str) -> Attempt | None:
    # Callers hold the task's lock already (lock_task), so this keeps the
    # task-then-attempt lock order.
    return db.scalar(
        select(Attempt)
