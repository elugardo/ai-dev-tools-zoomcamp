...
// an implementation from `useServices()` and never call fetch, import an
// implementation, or touch another data source. There are two implementations:
// ./http/httpService.ts calls the FastAPI backend (the default), and
// ./mock/mockService.ts runs entirely in the browser. services/index.ts picks one.
//
